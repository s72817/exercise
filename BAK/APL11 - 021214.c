...
struct kunde {
	...
	int PIN;
	int isBlocked;
}; // kunden[MAXKunde]; //MAXKunde Kunden können angelegt werden

struct kunde kunden[MAXKunde]; //initialisierung
...
DB Laden
------------------------------*/
int readDB(struct kunde k[], int *KundeID) {
	FILE *input;
	errno_t err;
...
int main(void) {
	...
	//KundenDB laden
	if (readDB(kunden, &KundeID) == 1) Status("DB geladen");
	...
	while (1) { //1.Menu
			...
			case 45:
			{
				...
				for (int i = 0; i < MAXKunde; i++)
					banksum += kunden[i].Guthaben;
				printf("\n\n----------------------------\n");
				printf("Das Gesamtumlaufvermoegen der Bank betraegt: %.2lf Euro\n", banksum);
				printf("----------------------------\n");
